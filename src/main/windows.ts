...
export const createHappWindow = async (
  ...
  if (uiSource.type === "path") {
    ...
    try {
      const iconResponse = await net.fetch(
        `http://localhost:${uiSource.port}/icon.png`
      );
  ...
  if (uiSource.type === "port") {
    try {
      ...
      console.error(
        `No index.html file found at http://localhost:${uiSource.port}/index.html`,
        e
      );
      if (is.dev && process.env["ELECTRON_RENDERER_URL"]) {
        ...
        happWindow.loadFile(
          path.join(__dirname, "../renderer/indexNotFound.html")
        );
      }
      happWindow.show();
      return happWindow;
...
export const createSplashWindow = (): BrowserWindow => {
  ...
  if (is.dev && process.env["ELECTRON_RENDERER_URL"]) {
    ...
    splashWindow.loadFile(
      path.join(__dirname, "../renderer/splashscreen.html")
    );
  }
